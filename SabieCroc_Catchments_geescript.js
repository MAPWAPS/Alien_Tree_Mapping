//Alien Map Classification of the SabieCroc Catchments

// Load study site from imports and zoom in//
...
var imageCol= S2.filterBounds(ROI)
.filterMetadata('CLOUD_COVERAGE_ASSESSMENT', 'less_than', 0.1)
.filterDate('2023-07-27', '2023-07-30')
print(imageCol)

// Spatially mosaic the scences, clip to mosaic to the ROI and display the image
// // Spatially mosaic the images
var mosaic= imageCol.mosaic()
Map.addLayer(mosaic, {opacity:0.2}, 'spatial mosaic')
print(mosaic, 'mosaic')

// // Clip image to SabieCroc
var Sabieclipped = mosaic.clip(ROI)
// print("Sabieclipped", Sabieclipped);
Map.addLayer(Sabieclipped, {bands: ['B4', 'B3', 'B2'], max: 3000}, 'Sentinel Image');

// rescale image bands /10 000
var image = Sabieclipped.divide(10000)
...
//ALOS LANDFORM
var landforms = ALOS_Landform.select('constant');
var landformsVis = {
  min: 11.0,
  max: 42.0,
  palette: [
...
var lfs = ALOS_Landform.select('constant');
Map.addLayer(landforms,landformsVis, 'landforms')

//ALOS DSM
var dataset= ee.ImageCollection(ALOS_DSM).mosaic()
...
// adding indices to the image as bands
...
var image_final=  image_final.addBands(lfs);
var image_final = image_final.addBands(elevation);
...
// select bands from the image to be used in training data
...
var classifier = ee.Classifier.smileGradientTreeBoost(100).train({
  ...
  classProperty: 'ID', //call from the training data
...
var SabieCrocPalette = [
  '351C75', //Bugweed//Persian Indigo
  'F91DF9', //Gum//Magenta
  '980A7D', // Lantana // Mardi Gras Purple
  '741b47', //Other invasive alien trees// Pansy Purple
  'fd0618',//Pine// Red
  'E06666', // Yellow Bells// Light Carmine Pink
  'ffcc99', // Bananas//Deep Peach
  ...
  'DB992D', // Macadamias/ Gamboge (deep yellow)
  'ff7f00', // Orchards// Orange
  'b7b7b7', // Shade/ grey
  '000000', //Urban//black
...
// Clip the classifion to ROI and display it  
var LULC_Clipped= classified.clip(ROI);
var viz= {min: 1, max: 18, palette: SabieCrocPalette}
Map.addLayer(LULC_Clipped, {min: 1, max: 18, palette: SabieCrocPalette}, 'classification');

//Export classification
Export.image.toDrive({
  image: LULC_Clipped,
  description: "SabieCrocClassification_v2",
  scale: 10,
  crs: 'EPSG:32736',
  region: ROI,
  maxPixels: 1e9,
...
Export.image.toAsset({
  image: LULC_Clipped,  
  description: 'sabiecroc_classification',
  assetId: 'projects/mapwaps-sabiecroc/assets/sabiecroc_classification',  // Set your asset path
  region: ROI,  //
...
// Export predicted versus observed data as csvs for training and validation datasets //
// to use in R for accuracy assessment and ploting spectral signatures //
...
Export.table.toDrive({
  ...
  description:'SabieClass1_6_validation_v2',
   fileFormat: 'CSV'
});
...
Export.table.toDrive({
  collection: training ,
  description:'SabieClass1_5_Training',
  fileFormat: 'CSV',
});
...
// name of the legend
var names = ['Alien_Bugweed','Alien_Gum', 'Alien_Lantana','Alien_Other', 'Alien_Pine','Alien_Yellow Bells','Bananas', 'Bare Ground', 'Burnt', 'Grassland', 'Indigenous Bush','Indigenous Forest',
          'Macadamias', 'Orchards','Shade','Urban', 'Water', 'Wetland' ]
// Add color and and names
for (var i = 0; i < 17; i++) {
  legend.add(makeRow(SabieCrocPalette[i], names[i]));
  }  
...
leftMap.addLayer(Sabieclipped, {bands: ['B4', 'B3', 'B2'], max: 3000}, 'S2 Image');


////////////////////////////////////////////////////////////////////////////////////////////
/////     Load classification from asset to avoid running the classification again   //////
////////////////////////////////////////////////////////////////////////////////////////////

// // Load the classification asset and assign it to LULC_Clipped
// var LULC_Clipped = ee.Image('projects/mapwaps-sabiecroc/assets/sabiecroc_classification');

// Add layers to the right map (classified map using the asset)
rightMap.addLayer(LULC_Clipped, {min: 1, max: 17, palette: SabieCrocPalette}, 'Classification');
rightMap.add(legend);

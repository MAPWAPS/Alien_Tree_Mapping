...
setwd("C:/Users/User/OneDrive - Stellenbosch University/MAPWAPS/Fieldwork/SabieCroc/FinalMapOutputs/Spectral unmixing")
...
bands_all = read.csv("Training_BandsIndices20250125.csv") %>% 
...
bands_all_grp$Landclass <- as.character(bands_all_grp$LULC_Class)
bands_all_grp$Landclass <- factor(bands_all_grp$Landclass, levels=unique(bands_all_grp$Landclass))
bands_all_grp <- bands_all_grp %>% mutate(Landclass = factor(Landclass, levels = c("Alien_Bugweed", "Alien_Gum", "Alien_Lantana","Alien_Other", "Alien_Pine","Alien_Yellow Bells" ,"Bananas", "Bare Ground","Burnt", "Grassland", "Indigenous Bush","Indigenous Forest", 
                                                                                   "Macadamias", "Orchards", "Urban", "Water", "Wetland" )))
levels(bands_all_grp$Landclass)

## add colour for alien tree other into here below "#7811F7",
cols = c("#351C75", "#F91DF9", "#980A7D", "#741b47", "#fd0618","#E06666", "#ffcc99", "#FFFFFF", "#999999", "#a8a800", "#6aa84f", "#14870e",
        "#DB992D", "#ff7f00", "#000000", "#0a14f9", "#08f3e4")
...
#scale_x_continuous(breaks = c(490, 560,   665,   705,  740, 786,   842,  865,  1610,   2190))
...
bands_full$Landclass <- as.character(bands_full$LULC_Class)
...
bands_full <- bands_full %>% 
  mutate(Landclass = factor(Landclass, 
                            levels = c("Alien_Bugweed", "Alien_Gum", "Alien_Lantana","Alien_Other", "Alien_Pine","Alien_Yellow Bells" ,"Bananas", "Bare Ground","Burnt", "Grassland", "Indigenous Bush","Indigenous Forest", 
                                      "Macadamias", "Orchards", "Urban", "Water", "Wetland" )))
